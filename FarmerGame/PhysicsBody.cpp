...
PhysicsBody::PhysicsBody(World* world, BodyMode bm, float x, float y, float degs)
	:world(world), body(nullptr)
{
	b2BodyDef boxDef;
	switch (bm)
	{
	...
	body = world->getPhysWorld()->CreateBody(&boxDef);
}
...
void PhysicsBody::draw()
{
	if (bodyBitMap != nullptr)
	{
		...
		int y = -(bodyPos.y - world->getMeterWorldY()) * B2D_SCALE;
		al_draw_scaled_rotated_bitmap(bodyBitMap, (bitmapW / 2.0), (bitmapH / 2.0), (bodyPos.x - world->getMeterWorldX()) * B2D_SCALE, y, (w * B2D_SCALE) / bitmapW, (h * B2D_SCALE) / bitmapH, -body->GetAngle(), NULL);
	}
}
...
PhysicsBody::~PhysicsBody()
{
	...
	joints.clear();
	world->getPhysWorld()->DestroyBody(body);
	if (bodyBitMap != nullptr)
	{
